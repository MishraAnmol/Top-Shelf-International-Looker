view: sales_by_order_prev_3months {
  derived_table: {
    sql: select invoice_month,fiscal_month_name_fy,net_sales_amount,
    lag(net_sales_amount,-1) over (partition by 1 order by invoice_month desc)
    as last_month_sales_amount,
      (lag(net_sales_amount,-1) over (partition by 1 order by invoice_month desc) +
      lag(net_sales_amount,-2) over (partition by 1 order by invoice_month desc) +
      lag(net_sales_amount,-3) over (partition by 1 order by invoice_month desc))/3 as last_3_month_avg
      from
      (SELECT
      ...
      WHERE (((sales_by_order."CUSTOMER_TYPE") = 'Direct Consumer')) AND ((sales_by_order."BRAND_NAME") = 'NED' OR (sales_by_order."BRAND_NAME") = 'Grainshaker' OR (sales_by_order."BRAND_NAME") = '3rd Party' OR (sales_by_order."ITEM_CODE") = 'Rebate')
      GROUP BY DATE_TRUNC('month', sales_by_order."INVOICE_DATE" ),2
      ORDER BY 1 DESC
      LIMIT 500)
       ;;
  }
